...
class Reservation extends Component {
    ...
    async obtainCalenderPermission() {
        ...
        if (permission.status !== 'granted') {
            ...
            if (permission.status !== 'granted') {
                Alert.alert('Permission not granted to set Calender for Rservation')
            }
        }
        return permission
    }

    async addReservationToCalender() {
        // debugger
        await this.obtainCalenderPermission()
        let calendars = await Calendar.getCalendarsAsync()
        let calendar = calendars.find(({ source }) => source.type === 'LOCAL')
        let startDate = new Date(Date.parse(this.state.date))
        let endDate = new Date(Date.parse(this.state.date) + (2 * 60 * 60 * 1000))
            ...
            <ScrollView>
                <Animatable.View animation='zoomIn' duration={2000} delay={1000} >
                    ...
                    <View style={styles.formRow}>
                        ...
                        <DatePicker style={{flex: 5, margin: 20}}
